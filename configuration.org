...
#+PROPERTY: header-args:emacs-lisp :tangle configuration.el :results silent
...
(setq custom-file (expand-file-name "automatic-settings.el" user-emacs-directory))
...
API keys and secrets live in =~/.emacs.d/.env= (gitignored, stored in vault).
This block reads the file on startup and injects each =KEY=value= pair into
Emacs's process environment so child processes (opencode, shell commands) pick
them up automatically.

#+begin_src emacs-lisp
(defun my/load-dotenv (&optional file)
  "Load KEY=VALUE pairs from FILE (default ~/.emacs.d/.env) into `process-environment'."
  (let ((env-file (or file (expand-file-name ".env" user-emacs-directory))))
...
(defun format-date-with-ordinal ()
  "Return current date as string with ordinal suffix e.g. 'Thursday, 19th March 2026'."
  (let* ((day (string-to-number (format-time-string "%e")))
    ...
    (format "%s, %d%s %s %s" day-name day suffix month year)))

(defun generate-org-header ()
    "Generate a formatted org-mode header with dynamic date and time."
    (let* ((formatted-date (format-date-with-ordinal))
           (time-with-zone (format-time-string "%H:%M:%S %Z"))
           (iso-date (format-time-string "%Y-%m-%d"))
           (timestamp (format-time-string "<%Y-%m-%d %a %H:%M>"))
           (author-name "Vsevolod Prudius")
           (author-email "i@sevap.ru"))
      (concat
       ";; Hope you have a good day!\n"
       ";; " formatted-date " | " time-with-zone "\n\n"
       ...
       "#+KEYWORDS: emacs, org-mode, documentation\n"
       "#+LANGUAGE: en\n"
       "#+OPTIONS: toc:t num:t H:4 ^:nil _:nil tags:t\n"
       "#+STARTUP: content fold\n"
       "#+EXPORT_SELECT_TAGS: export\n"
       "#+EXPORT_EXCLUDE_TAGS: noexport draft\n"
       "#+FILETAGS: :project:\n"
       "\n"
...
(setq-default inhibit-startup-screen t
                ...
                initial-scratch-message (generate-org-header))
...
** Multilanguage Keybindings

When typing in a non-Latin script (e.g. Russian/Cyrillic) at the OS level, Emacs
receives the Cyrillic character codes and keybindings stop working. The fix maps
each foreign character to the Latin character at the same physical key position, so
=C-ф= becomes =C-a=, =M-в= becomes =M-d=, etc. — regardless of OS layout.

The mapping covers the full Russian ЙЦУКЕН → QWERTY layout for bare keys and all
common modifier combinations (Control, Meta, Control-Meta).

#+begin_src emacs-lisp
(defun my/setup-multilang-keybindings ()
  "Translate Cyrillic key events to their QWERTY positional equivalents.
Allows all keybindings to work regardless of the active OS keyboard layout."
  (let ((pairs
         '(;; top row
           (?й . ?q) (?ц . ?w) (?у . ?e) (?к . ?r) (?е . ?t)
           (?н . ?y) (?г . ?u) (?ш . ?i) (?щ . ?o) (?з . ?p)
    ...
    (dolist (pair pairs)
      (let ((ru (car pair))
            (en (cdr pair)))
        (dolist (mod '(() (control) (meta) (control meta)))
          (define-key key-translation-map
            (vector (append mod (list ru)))
            (vector (append mod (list en)))))))))

(my/setup-multilang-keybindings)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq calendar-latitude    (string-to-number (or (getenv "CALENDAR_LATITUDE")  "52.36547"))
        calendar-longitude   (string-to-number (or (getenv "CALENDAR_LONGITUDE") "4.81926"))
        calendar-location-name (or (getenv "CALENDAR_LOCATION") "Amsterdam")
        calendar-time-zone   (string-to-number (or (getenv "CALENDAR_TZ_OFFSET") "60")))
...
Shows available key bindings after pressing a prefix key. Zero friction, universal value.
...
Supercharges vertico with live preview, project-wide grep, buffer switching, etc.
Pairs perfectly with the existing vertico + orderless stack.
...
Right-click-like context actions on any candidate in the minibuffer or at point.
...
corfu gives a lightweight in-buffer completion popup. cape is already installed
but unconfigured — this wires it up properly.
...
Zero-dependency LSP client built into Emacs 29+. Enables go-to-def, find-refs,
rename, and diagnostics for any LSP-supported language.
...
Shows +/- in the margin for changed lines. Complements magit perfectly.
...
Jump to any visible character with 2-3 keystrokes. Navigation superpower.
...
Rich status bar with icons, git branch, LSP status, and encoding info.
Requires ~nerd-icons~ and running =M-x nerd-icons-install-fonts= once.
...
Navigate undo history as a visual tree. Built on Emacs native undo, no data loss.
...
Edit grep/consult search results directly in buffer, apply changes to multiple files.
...
Full-featured terminal emulator needed for TUI applications like opencode.
Requires =libvterm= system library (=sudo apt-get install libvterm-dev cmake=).
...
[[https://github.com/yuya373/claude-code-emacs][claude-code]] integrates the Claude Code CLI into Emacs via vterm, with
project-isolated sessions and a transient menu.

#+begin_src emacs-lisp
(use-package claude-code
  :after vterm
  :bind ("C-c C-a" . claude-code-transient))
#+end_src

** Opencode Buffer

=M-x opencode= launches the opencode TUI in a dedicated vterm buffer, with
credentials loaded from =~/.emacs.d/.env= at Emacs startup (see =my/load-dotenv=).
Reuses an existing session if one is running.

#+begin_src emacs-lisp
(defun opencode ()
  "Launch opencode in a dedicated vterm buffer, reusing existing live session."
  (interactive)
  (let ((buf-name "*opencode*")
        (buf (get-buffer "*opencode*")))
    (if (and buf (get-buffer-process buf))
        (switch-to-buffer buf)
      (let ((vterm-shell (executable-find "opencode")))
        (unless vterm-shell
          (user-error "opencode not found in PATH — install from https://opencode.ai"))
        (vterm buf-name)))))
#+end_src
